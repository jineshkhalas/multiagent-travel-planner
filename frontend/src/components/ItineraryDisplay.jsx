...
export default function ItineraryDisplay({ content }) {
  ...
  const MarkdownComponents = {
    h1: ({ children }) => {
      return (
        <div className="flex items-center flex-wrap gap-2.5 mt-6 mb-4 pb-2 border-b border-gray-200 dark:border-gray-800 text-gray-900 dark:text-gray-100 font-bold text-base sm:text-lg font-heading">
    ...
    h2: ({ children }) => {
      ...
      return (
        <div className="flex items-center justify-between flex-wrap gap-2 mt-7 mb-3.5 pt-3.5 pb-2 border-b border-gray-200/80 dark:border-gray-800 font-heading">
    ...
    h3: ({ children }) => {
      ...
      return (
        <div className="flex items-center justify-between flex-wrap gap-2 mt-6 mb-3 pb-2 border-b border-gray-200/90 dark:border-gray-800 font-heading">
    ...
    h4: ({ children }) => {
      ...
      return (
        <div className="flex items-center gap-2 mt-5 mb-2.5 text-gray-900 dark:text-gray-100 font-semibold text-xs sm:text-sm tracking-wide font-heading">
          {icon}
          <span>{children}</span>
        </div>
      );
    ...
    p: ({ children }) => (
      <div className="text-gray-700 dark:text-gray-300 text-[13.5px] sm:text-[14.5px] leading-relaxed mb-3.5 break-words font-normal">
        {children}
      </div>
    ),

    li: ({ children }) => (
      <li className="text-gray-700 dark:text-gray-300 text-[13.5px] sm:text-[14.5px] mb-2.5 leading-relaxed pl-1 break-words font-normal">
        {children}
      </li>
    ),
    ...
    strong: ({ children }) => {
      ...
      if (/^day\s*\d+/i.test(text)) {
        return (
          <span className="inline-flex items-center gap-1.5 font-bold text-blue-900 dark:text-blue-200 bg-blue-50 dark:bg-blue-950/50 border border-blue-200 dark:border-blue-800/60 px-2.5 py-1 rounded-lg text-xs mr-2 my-1 shadow-xs shrink-0 font-heading">
    ...
    ul: ({ children }) => (
      <ul className="space-y-3 my-3 pl-4 sm:pl-5 list-disc marker:text-blue-500 dark:marker:text-blue-400 text-[13.5px] sm:text-[14.5px]">
        {children}
      </ul>
    ),

    ol: ({ children }) => (
      <ol className="space-y-3 my-3 pl-4 sm:pl-5 list-decimal marker:font-semibold marker:text-blue-600 dark:marker:text-blue-400 text-[13.5px] sm:text-[14.5px]">
        {children}
      </ol>
    ),

    blockquote: ({ children }) => (
      <blockquote className="border-l-4 border-blue-500 dark:border-blue-400 bg-blue-50/60 dark:bg-blue-950/40 p-3 sm:p-4 my-3.5 rounded-r-lg text-xs sm:text-sm text-gray-700 dark:text-gray-300 italic flex gap-2 items-start leading-relaxed">
